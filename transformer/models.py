...
class MultiCNNLSTM(nn.Module):
    def __init__(self, mods, 
                 ...
                 cnn_embed_sizeL=128, cnn_embed_sizeA=300, cnn_embed_sizeE=15,
                 window_embed_size=128, k=5,
                 device=torch.device('cuda:0')):
        super(MultiCNNLSTM, self).__init__()
        self.mods = mods
        self.CNN_L = CNN(word_embed_sizeL, cnn_embed_sizeL, k)
        self.CNN_A = CNN(word_embed_sizeA, cnn_embed_sizeA, k)
        self.CNN_E = CNN(word_embed_sizeE, cnn_embed_sizeE, k)
        self.Highway_L = Highway(cnn_embed_sizeL)
        self.Highway_A = Highway(cnn_embed_sizeA)
        self.Highway_E = Highway(cnn_embed_sizeE)
        self.LSTM = NLPTransformer(cnn_embed_sizeL+cnn_embed_sizeA+cnn_embed_sizeE)
